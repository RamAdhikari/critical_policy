class critical_policy {
  ...
  onlyif    =>'if(((((((get-acl $TargetFolder).access)| select IdentityReference | ft -HideTableHeaders | Out-String).trim()) -replace "\`r\`n", "" ) -replace "              ",",") -ne "Everyone,BUILTIN\Administrators")',
  provider  => powershell,
  }
  exec { 'rename-guest':
